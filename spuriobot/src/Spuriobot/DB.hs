...
data Failure = Failure Int64 FailType UTCTime Text (Maybe Int64) (Maybe Text) Text
...
insertFailures x conn = executeMany conn
    [sql|
        insert into ci_failure (job_id, type, job_date, web_url, runner_id, runner_name, job_name)
        values (?, ?, ?, ?, ?, ?, ?)
